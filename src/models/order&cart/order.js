...
import { getRestaurantIdFromSlug } from './cart.js';
...
const saveNewOrder = async (userId, resSlug) => {
  ...
  const result = await db.query(orderQuery, [userId, resId, 1, prices.subtotal, prices.total, prices.deliveryFee, order.deliveryMinutes]);
  const orderId = result.rows[0].orderId;
  const orderDishQuery = `INSERT INTO order_dish(order_id, dish_id, quantity, order_price) 
                            VALUES ($1, $2, $3, $4) `;

  for (const dish of order.dishes) {
    const orderDishResult = await db.query(orderDishQuery, [orderId, dish.dishId, dish.quantity, dish.price]);
  }
...
const updateOrderStatus = async (orderId, status) => {
    const statusIdResult = await db.query( `SELECT id FROM order_status WHERE LOWER(status) = LOWER($1)`, [status]);
...
export {getUserDishHistorybyRest, isResOpen, saveNewOrder, getOrderById, updateOrderStatus};